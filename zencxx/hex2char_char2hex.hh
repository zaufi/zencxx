 ...
 */
constexpr inline auto hex2char(const char c)
{
 ...
 * \todo How to prevent invalid chars? What to produce? Nulls??
 */
constexpr inline auto hex2char(const char high, const char low)
{
    return (hex2char(high) << 4) + hex2char(low);
}
/// Return hex encoded low tetrade of char
constexpr inline auto char2hex_l(const char c)
{
    return (c & 0x0f) > 9 ? (c & 0x0f) - 10 + 'A' : (c & 0x0f) + '0';
}
/// Return hex encoded high tetrade of char
constexpr inline auto char2hex_h(const char c)
{
...
constexpr inline auto char2hex(const char c)
{
